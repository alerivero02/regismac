// Envolver TODO en try-catch para capturar errores de importación del módulo
let handlerFunction;

try {
  // Intentar importar Prisma, pero no fallar si no está disponible
  let PrismaClient;
  try {
  ...
  handlerFunction = async function handler(req, res) {
  ...
  // Si hay un error al cargar el módulo, crear un handler mínimo
  console.error('❌ ERROR CRÍTICO AL CARGAR MÓDULO:', moduleError);
